...
class TourController extends Controller
{
    ...
    {
        $categorias = Categoria::with('image','toursweb')->withCount('toursweb')->where('language_id',$request->language_id)->get();
    ...
    {
        $categoria = Categoria::where('slug',$request->slug)->with('image','toursweb')->withCount('toursweb')->where('language_id',$request->language_id)->first();
    ...
    {
        $ubicaciones = Ubicacion::with('image','toursweb')->withCount('toursweb')->where('language_id',$request->language_id)->get();
    ...
    {
        $ubicacion = Ubicacion::where('slug',$request->slug)->with('image','toursweb','tours.ubicaciones')->withCount('toursweb')->where('language_id',$request->language_id)->first();
    ...
    public function general(Request $request)
    {
        ...
        $paises = Pais::all();
        $menu = Menu::with('detalles','detalles.categoria','detalles.ubicacion','detalles.ubicacion.toursweb','detalles.categoria.toursweb')->where('estado',1)->where('language_id',$request->language_id)->get();
        $data = [
            "nosotros" => $nosotros,
            "cabecera" => $cabecera,
