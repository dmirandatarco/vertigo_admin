...
class VenderPaquete extends Component
{
    public $celular;
    public $paises;
    public $email;
    ...
    public $tipoReserva=1;
    public $userId;
    public $users;

    public function mount(Paquete $paquete)
    {
        ...
        foreach($paquete->detalles as $i => $detalle){
            $this->cantidad[$i] = $paquete->cantidad;
            $this->ingreso[$i] = 0;
            $this->fecha_viaje[$i] = $detalle->fecha;
            $this->precio[$i] = $detalle->precio;
            $this->observacion[$i] = $detalle->observacion;
            $this->tour_id[$i] = $detalle->tour_id;
            $this->moneda_id[$i] = $paquete->moneda_id;
            $this->hotel_id[$i] = "";
        }
        if($paquete->moneda_id == '1'){
            $this->totalsoles=$paquete->total;
            $this->reservasoles=$this->totalsoles;
            $this->reservadolares=0;
        }
        if($paquete->moneda_id == '2'){
            $this->totaldolares=$paquete->total;
            $this->reservasoles=0;
            $this->reservadolares=$this->totaldolares;
        }
        $this->cont2=count($paquete->servicios);
        foreach($paquete->servicios as $i => $servicio){
            $this->cantidadsevicio[$i] = $servicio->pivot->cantidad;
            $this->servicio_id[$i] = $servicio->id;
            $this->precio_operacion[$i] = $servicio->pivot->precio;
            $this->descripcion[$i] = "";
        }
    }
    ...
    public function aumentarPago()
    {
        ...
        if($this->cont3==0){
            ...
            $this->cuentas=Medio::select('id','nombre as text')->whereRelation('moneda','id','=',1)->get();
            $this->push=collect($this->push);
            $datos=$this->push->concat($this->cuentas);
            $this->cont3++;
    ...
    {
        return view('livewire.vender-paquete');
    }
}
