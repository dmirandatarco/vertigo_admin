...
class EditarPaquete extends Component
{
    ...
    {
        return view('livewire.editar-paquete');
    }

    public function mount(Paquete $paquete)
    {
        $this->paquete = $paquete;
        $this->tours = Tour::where('estado', 1)->where('language_id',$this->paquete->language_id)->get();
    ...
    {
        $idioma=Language::find($this->paquete->language_id);
        try {
            DB::beginTransaction();
            $this->validate([
            ...
            // $paquete = $user->paquetes()->where('id', $this->paquete->id)->first();
                $paquetenuevo = $user->paquetes()->find($this->paquete->id);
                ...
                $paquetenuevo->update([
                    ...
                    'language_id'   => $this->paquete->language_id,
                    'user_id'       => \Auth::user()->id,
                ]);
            $paquetenuevo->detalles()->delete();
            for ($i = 0; $i < $this->cont; $i++) {
                $paquetenuevo->detalles()->create([
            ...
            $paquetenuevo->servicios()->detach();

            for ($i = 0; $i < $this->cont2; $i++) {
                $paquetenuevo->servicios()->attach($this->servicio_id[$i], ["cantidad" => $this->cantidadsevicio[$i]]);
            }
            DB::commit();
        } catch (Exception $e) {
            DB::rollBack();
