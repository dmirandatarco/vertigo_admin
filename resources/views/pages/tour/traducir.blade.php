...
@endpush
@section('content')
<form action="{{route('tour.guardartraducido',$tour)}}" method="post" class="forms-sample" enctype="multipart/form-data">
  {{csrf_field()}}
<div class="card mb-3">
  <div class="card-header">
    <h2 class="text-primary">Traducir Tour</h2> 
  </div>
</div> 
    ...
    <div class="card">
      <div class="card-body">
        <div class="row">
          ...
          <div class="mb-3 col-md-6" >
            ...
            <select class="form-select js-states" id="categoria_id" name="categoria_id" data-width="100%">
              ...
              @foreach($categorias as $categoria)
                <option value="{{$categoria->id}}" @selected(old('categoria_id',$tour->categoria->traduccion($lang)?->id)==$categoria->id)>{{$categoria->nombre}}</option>
              @endforeach  
            </select>    
            @error('categoria_id')
              <span class="error-message" style="color:red">{{ $message }}</span>
            ...
            <select class="form-select" id="ubicacion_id" name="ubicacion_id[]" data-width="100%" multiple>
              @foreach($ubicaciones as $ubicacion)
                  <option value="{{$ubicacion->nombre}}" 
                    ...
                    @else
                      @foreach($tour->ubicaciones as $ubicaciond)
                        @selected($ubicaciond->traducciones->contains($ubicacion) || $ubicaciond->traduccionesinversas->contains($ubicacion))
                      @endforeach
                    @endif
          ...
          <div class="mb-3 col-md-6" >
            <label class="form-label" for="precio_confidencial">Precio Confidencial:</label> 
    ...
    <div class="card">
      <div class="card-body">
        <div class="row">
          ...
          <div class="mb-3 col-md-6" >
            <input type="hidden" name="language_id" value="{{$lang}}">
          </div>
        </div>
      </div>
    </div>
