...
<div class="row inbox-wrapper">
    <div class="col-md-12">
        ...
        <div class="card-body">
            <div class="row">
            <h4 class="mb-3">Listado de Certificados</h4>
            <div class="mb-5">
            </div>
            ...
            <div class="table-responsive">
                ...
                @foreach($certificados as $i => $certificado)
                    <tr>
                        ...
                        <td> {{$certificado->user->nombre}} {{$certificado->user->apellido}}</td>
                        <td><img style="width:25px;" src="{{$certificado->language?->icono}}" alt="{{$certificado->language?->icono}}"></td>
                        @foreach($languages2 as $language)
                            <td>
                            @php
                                $translation = $certificado->traducciones->firstWhere('language_id', $language->id);
                                $inverseTranslation = $certificado->traduccionesinversas->firstWhere('language_id', $language->id);
                                $inverseTranslationespanol = $certificado->traduccionesinversas->firstWhere('language_id', 1);
                                $selectedTranslation = $translation ?? (($inverseTranslation?->language_id == $language->id) ? $inverseTranslation : (($inverseTranslationespanol && $certificado->language_id == $language->id) ? $inverseTranslationespanol : null ));
                            @endphp
                            ...
                            @else
                                <a href="{{ route('certificados.traducir',['lang'=>$language->id,'certificados'=>$certificado]) }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{route('certificados.destroy','test')}}" method="POST" autocomplete="off">
                        {{method_field('delete')}}
                        {{csrf_field()}}
                        <p>Estas seguro de cambiar el estado?</p>
                        <div class="modal-footer">
                            <input type="hidden" name="id_certificados_2" class="id_certificados_2">
...
eliminar.addEventListener('show.bs.modal', function(event) {
    ...
    var idModal = eliminar.querySelector('.id_certificados_2')
    idModal.value = id;
});
